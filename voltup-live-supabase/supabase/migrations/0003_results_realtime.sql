...
-- 화면은 원시 votes 를 받지 않는다. 집계 RPC(초기/재연결 fetch) + broadcast(라이브 갱신)만 쓴다.

-- 질문별 집계 --------------------------------------------------------------
-- SECURITY DEFINER: votes 의 SELECT RLS 를 우회해 count 만 반환. 원시 행/voter_key 미노출.
...
begin
  ...
  -- q_type = 'text' : 정규화(소문자+trim) 빈도, 상위 100. total 은 전량 기준.
  select count(*)::int
    into total_count
    from public.votes
...
-- 투표 INSERT 시 집계를 broadcast (원시 row 아님, 집계 payload 만) -----------
-- realtime.send(payload jsonb, event text, topic text, private boolean)
-- private=false → 공개 채널. /screen · /admin 이 'results:{question_id}' 구독.
...
-- sessions 만 Postgres Changes 로 스트림. votes 는 스트림하지 않는다(위 broadcast 로 대체).
do $$
begin
