...
| `layout.cloudWidth` | `min(100%, max(58vh, 42vw))` | 단어가 감기는 폭 |
| `layout.cloudWords` | `30` | 상위 N |
| `layout.cloudFitWords` | `13` | 이 개수일 때 최대 크기 |
...
- **색은 두 개뿐**: 최다 단어 `blue`, 나머지 `ink`. 회색·투명도·무지개 금지.
  **위계는 크기와 굵기로만 만든다.** 굵기는 중앙값 이상 700, 미만 600.
- **배경 없음**: 컨테이너에 background 도 border 도 두지 않는다. 글래스 위에 텍스트만 얹는다.
