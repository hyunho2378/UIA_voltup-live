...
**링크(`https://uia-vote.vercel.app/`)를 열면 표지가 뜬다. QR 이 아니다.** 표지에는 "코리아 넥스트 임팩트 포럼 /
INTERACTIVE SESSION / 청중과 함께 그려보는 미래의 대학" 만 있다. 우측 하단 "메뉴" 를 누르면 대형화면·관리자·투표 화면
목록이 열리고, 거기서 관리자로 들어간다. **관리자에서 "QR 열기" 를 눌러야 그때 대형화면이 QR 로 바뀐다.**
청중은 그 QR 로만 들어온다. 이후 질문을 고르고 "투표 열기" → "결과 공개" 순으로 진행한다. 표지로 되돌리려면
관리자 "표지 화면" 을 누른다. 대형화면을 프로젝터에 띄운 뒤 **F 키** 를 누르면 전체화면이 되고 그동안 메뉴 버튼은
사라진다(다시 F 또는 ESC 로 해제). 세션 기본 상태는 표지이고, "전체 초기화" 도 표지로 되돌린다.
