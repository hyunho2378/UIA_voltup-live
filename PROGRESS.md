...
**링크(`https://uia-vote.vercel.app/`)를 열면 청중 투표 화면이 뜬다**(2026-09-22 변경. 청중이 도메인으로
직접 들어오는 경우가 많아 기본값을 투표로 돌렸다). **대형화면은 `https://uia-vote.vercel.app/screen` 으로 연다.**
대형화면 우측 하단 "메뉴" 로 관리자에 들어가고,
**관리자에서 "QR 열기" 를 눌러야** 대형화면이 QR 로 바뀐다. 청중은 그 QR 로만 들어온다.
이후 질문 1 → 2 → 3 → 4 순으로 진행한다. **질문 목록에서 질문을 누르면 투표가 바로 열린다**(2026-09-21부터.
"투표 열기" 를 따로 누르는 단계를 없앴다. 시간이 없는 현장에서 클릭 한 번을 아끼기 위한 변경). 다 걷혔으면
"투표 닫기" 로 마감하고 "결과 공개" 로 띄운다(결과 공개를 누르는 순간 집계가 확정 전송되므로 숫자가 정확해진다).
**4번은 결과 공개 시 워드클라우드로 자동 표시**한다. 막대 전환 버튼은 없다. 교수님 마무리 뒤에는 **"경주포럼 화면"**(2026 세계경주포럼 홍보 + 참가등록 QR)을 띄우고, 마지막에 **"클로징 화면"**(사진 촬영용 배경)으로 넘긴다. 두 버튼은 나란히 있고 순서는 진행자가 정한다.
...
## 질문 2 선택지 교체 + 워드클라우드 글래스 칩 (2026-09-20)

### 질문 2 선택지

큐시트 수정본대로 6개에서 5개로 바꿨다.
...
SOURCE.md, seed.sql, _ALL.sql, 운영 DB 전부 맞췄다. 5행이라 앞서 6행에서 겪던 세로 넘침 문제는 없다.

### 워드클라우드 글래스 칩

단어를 버튼처럼 감싸 달라는 요청. `.cloud-chip` 을 추가했다.

WebGL 글래스는 쓸 수 없다. 라이브러리가 GlassRoot 직계 자식만 처리하는데 단어는 패널 안쪽 깊이 있고,
단어 30개에 WebGL 컨텍스트를 할당하면 브라우저 한도(약 16개)를 넘긴다. 그래서 CSS 글래스 폴백과 같은
재질 값을 쓴다. padding 을 `em` 으로 둬서 글자가 커지면 칩도 같이 커진다. radius 는 `full` 이다.
셰이더를 타지 않는 CSS 요소라 `pill` 을 강제하는 제약이 적용되지 않는다.

단독 1등만 `blue` 필 + `white` 글자, 나머지는 글래스 칩 + `ink` 글자다. 색 규칙은 막대와 같다.

**칩을 입히니 세로로 240px 넘쳤다.** 좌우 1.2em, 상하 0.52em 이 단어마다 더 붙어 같은 단어 수에서
줄이 9줄까지 늘어난 탓이다. `cloudWidth` 를 `max(58vh, 42vw)` 에서 `max(76vh, 58vw)` 로 넓히고
`cloudFitWords` 를 13에서 8로 낮춰 전체 크기를 줄였다. 수정 후 16단어 기준 8줄, 넘침 0, 겹침 0.

**성능 영향 없다.** 칩 16개 + WebGL 글래스 상태에서 25건을 흘려 넣으며 프레임을 쟀다.
중앙값 8.3ms, p99 9.9ms, 최대 31.2ms, **33ms 초과 프레임 0회.** 막대 뷰에서 관측된 재캡처 끊김이
칩 때문에 나빠지지는 않았다.

산출물: `K-cloud-chips.png`, `K-q2-new.png`


## 대형 선택지와 Q4 전용 워드클라우드 수정 (2026-09-20)

사용자 제보: 투표를 열어도 대형화면에 객관식 선택지가 없고, Q4 칩은 너무 크며 가운데 정렬이 어색함. Q4 막대 불필요.

- 이전: 결과 공개 전 질문만 표시. 이후: 객관식 선택 직후 OptionList 표시, 집계는 공개까지 숨김.
- screenOption: clamp(32px, 4vw, 80px), weight 700. 1440px 실측 57.6px. 1920px 계산값 76.8px.
- cloudMax: clamp(64px, 8vw, 154px)에서 clamp(46px, 5vw, 96px)로 축소. 칩 nowrap + center, 긴 단어의 계산 크기 축소. 최소 크기는 길이 보정에서 제외.
- Q4: results_view 값과 무관하게 워드클라우드 고정. 관리자 및 개발 패널의 막대/클라우드 버튼 제거.
- 빌드 성공. 1440x900 격리 화면: 객관식 5개, 가로/세로 넘침 0. Q4 19단어 중심 x=720, 화면 중심 x=720, 막대 0, 가로/세로 넘침 0.
- 검증용 one-/mix- voter_key 31건만 식별해 삭제, 정리 직전 그 외 응답 0건. 운영 세션은 Q4 상태 유지. 앞선 재현 과정에는 전체 투표 삭제가 수행됐으므로 이전 응답 보존을 주장하지 않음.
- 실제 1920x1080 및 모바일/4K 물리 뷰포트는 이 환경에서 미검증.
- 운영 배포 완료: 8724d58. https://uia-vote.vercel.app/ HTTP 200, 배포 JS index-ChHitSsc.js와 CSS index-B_kq2WcK.css 확인. 선택지 clamp(32px,4vw,80px)/700, cloudMax 96px, nowrap, 뷰 전환 제거가 운영 번들에 포함됨. /screen에서 Q4 질문 로딩 정상 확인. 다음: 현장 프로젝터에서 크기 최종 확인.


## 표지 브랜드 자산 + 선택지 카드 + 단어 수 자동 축소 (2026-09-20)

요청 세 가지. (1) 단어가 많아지면 알아서 줄어들 것, (2) 선택지 사이 선이 안 보이니 항목마다 섹션으로 감쌀 것,
(3) 로고·타이틀·그래픽 SVG 를 표지에 넣되 색을 디자인 시스템에 맞출 것.

### 1. 실측 맞춤 (`useFitScale`)

계산식만으로는 안 맞았다. 같은 단어 수라도 글자 수와 줄바꿈 위치에 따라 줄 수가 갈린다.
그려 놓고 `scrollHeight > clientHeight` 면 `--fit` 을 0.92배씩 최대 8회 줄인다. `ResizeObserver` 로
창 크기 변화도 같은 경로로 처리한다. 워드클라우드와 선택지 목록이 같은 훅을 쓴다.

**전제 조건이 하나 있었다.** `.glass-root` 가 `min-height: 100dvh` 라 패널 높이가 내용에 끌려갔고,
컨테이너의 `clientHeight` 가 곧 내용 높이라 비교가 성립하지 않았다(실측 22px 초과, `--fit` 은 1 그대로).
`/screen` 에만 `.glass-root-fixed`(height 100dvh)를 걸고 중간 래퍼에 `min-h-0` 을 뚫어 해결했다.

1440x900 실측: 3/8/14/20/26/30단어 전 구간 겹침 0, 넘침 0. 최대 단어 72px → 37px 로 단계적 축소.
1920x1080 실측(프레임 하네스): 30단어 42~50px, 겹침 0, 넘침 0.

### 2. 선택지 카드

행 사이 hairline(`rowLine` 0.08)은 20m 프로젝터에서 사라진다. 선을 진하게 하는 대신 항목마다 면을 줬다.
`.option-card` 는 워드클라우드 칩과 같은 CSS 글래스 재질, radius `lg`. 아직 아무도 고르지 않은 상태라
필을 채우지 않는다. 채우면 선택된 항목처럼 읽힌다. 결과 막대 쪽 `rowLine` 은 0.08 → 0.16 으로 올렸다.

1920x1080 실측: 카드 5개, 글자 71px(fit 0.92), 넘침 0. 1440x900: 53px(fit 0.92), 넘침 0.

### 3. 표지 브랜드 자산

원본 색이 로고 16개, 그래픽 500개, 타이틀 1개였다. 색상은 버리고 **명도만 남겨** sRGB 상대 휘도를
`colors.brandRamp` 6단(blue 계열) 위에 다시 앉혔다. `npm run brand`(`scripts/brand-assets.mjs`).

- 원본 `brand-src/`, 산출물 `client/public/images/brand/`. 로고 6KB, 타이틀 11KB, 그래픽 55KB(WebP).
- 그래픽은 요소 7,956개라 SVG 로 넣지 않는다. 재도색 후 WebP 1100px 로 굽는다. 배포본 총 704KB.
- 로고 drop-shadow 필터 제거(그림자는 글래스 재질만 낸다).
- 타이틀은 처음 받은 파일이 SEBANG Gothic 생텍스트라 폰트 미설치 시 깨졌다. 사용자가 아웃라인 버전으로
  다시 줬다(패스 28개, 단색). 그걸 `ink` 로 바꿔 쓴다.
- 세 자산 모두 글래스 패널 안에 둔다. 뒤에 두면 굴절·blur 로 형태가 뭉개지고, 라이브러리가 GlassRoot
  직계 자식만 배경으로 잡아 중간 래퍼도 못 둔다.

표지 실측 1920x1080: 로고 173x90, 타이틀 820x419, 그래픽 420x813, 가로/세로 넘침 0.
1024x768 에서도 넘침 0.

### 검증

| 항목 | 1440x900 | 1920x1080 |
|---|---|---|
| 표지 3자산 로드 | 정상 | 정상, 넘침 0 |
| 선택지 카드 5개 | 53px, 넘침 0 | 71px, 넘침 0 |
| 워드클라우드 30단어 | 32~37px, 겹침 0 | 42~50px, 겹침 0 |
| 결과 막대 | 5개, rowLine 0.16 | 5개, rowLine 0.16 |

실제 프로젝터 물리 검증은 미수행. 브라우저 창 기준 측정이다.


## 큐시트 최종 대본 반영 + 표지 모바일 대응 (2026-09-21)

### 질문 4개 전체 교체

사용자가 실제 진행 대본(사회자 대사 포함 전체 큐시트) 전달. Q1~Q4 문항과 선택지를 대본 문구로 교체했다.
Q2 가 5지→6지로 늘었다("스스로 배우고 진로를 설계하는 역량" 추가). SOURCE.md, seed.sql, _ALL.sql, 운영 DB
전부 맞췄다(질문 4건 PATCH 204, 선택지 15건 PATCH 204, Q2 6번 옵션 신규 POST 201).

Q4 는 화면에 문구가 두 줄로 명시돼 있었다(질문형 + 빈칸 채우기형). 질문은 DB `title`, 빈칸 문장은
DB 컬럼을 새로 만들지 않고 `type==='text'` 조건부 상수(`TEXT_CAPTION`)로 Vote.jsx/Screen.jsx 양쪽에 뒀다.
이 앱에 주관식 질문이 Q4 하나뿐이라 타입 조건만으로 안전하다.

**버그 하나 잡았다.** `<Ko>“{TEXT_CAPTION}”</Ko>` 처럼 문자열+표현식+문자열을 JSX children 으로 나누면
React 가 배열로 넘기고, `Ko` 내부의 `String(children)` 이 `String(array)` 가 되어 쉼표로 이어붙었다
(실측: `“,내가 생각하는...,”`). `` `“${TEXT_CAPTION}”` `` 처럼 템플릿 리터럴로 하나의 문자열로 합쳐서 해결.
같은 세션에서 처음 만든 코드라 큐시트 문안이 아닌 다른 `<Ko>` 호출부는 전부 단일 표현식이라 이 결함이 없었다.

### 표지 모바일 대응

문제 제보: "관객들이 저렇게 보면 안되지", 모바일에서 그래픽 위치가 이상함. 원인은 /screen 이 1920 기준으로만
설계돼 있어 좁은 뷰포트에서 로고·타이틀·그래픽 2단 가로 배치가 그대로 눌렸다.

- `lg`(1024) 미만은 `flex-col` 로 세로 스택(로고→타이틀→모듈 태그→헤드라인→그래픽). 1024 이상은 기존 가로 2단.
- 그래픽 크기를 lg 기준으로 분리했다. lg 이상 `coverGraphicMax`(min(30%,420px))/`coverGraphicMaxH`(78vh),
  lg 미만 `coverGraphicMaxSm`(min(58%,280px))/`coverGraphicMaxHSm`(26vh). 가로 배치의 "패널 폭 30%" 규칙이
  세로 스택에서는 의미가 없다.
- **패널 여백도 줄였다**(`p-lg lg:p-4xl`). 48px 여백을 양쪽에 그대로 두면 320px 폭에서 내용 폭이 128px밖에
  안 남아 한글이 5~7자마다 줄바뀜하고 그래픽이 스크롤 밖으로 밀렸다(최초 시도에서 실측). p-lg(20px)로 줄이니
  320px 에서도 스크롤 없이 로고·타이틀·태그·헤드라인·그래픽이 한 화면에 들어간다. 1024 이상은 원래 48px 그대로라
  기존 1920 검증에 영향 없다.

### 타이틀·모듈 태그

- h1 강제 줄바꿈 마커(` / `) 제거. "청중과 함께 그려보는 미래의 대학"을 자연스러운 `balance` wrap 에 맡겼다.
  1920 에서 한 줄, 390 에서 두 줄로 균형 있게 갈린다(실측).
- 로고 축소: `coverLogoMax` clamp(96,9vw,190) → clamp(64,6vw,130).
- 타이틀 축소: `coverTitleMax` min(52vw,820px) → min(40vw,600px). 로고-타이틀 간격도 `mt-2xl`(32px) →
  `mt-4xl`(48px)로 넓혀 두 자산이 붙어 보이지 않게 했다.
- **모듈 태그 추가.** 이 세션이 포럼 전체가 아니라 5모듈 하나라는 걸 표시하려고 로고·타이틀 아래,
  헤드라인 위에 2행을 넣었다. 1행 "MODULE 05"(영문, 좁지 않은 트래킹, `screenModuleEn`), 2행
  "인터렉티브 세션"(한글 강조, `screenModuleKr`, 큐시트 사회자 대사 표기 그대로). 두 줄 사이 간격을
  `mt-md`(12px)로 명시해 기본 행간보다 분리했다.

### 검증

| 항목 | 320x690 | 390x844 | 1024 경계 | 1920x1080 | 1440x900 |
|---|---|---|---|---|---|
| flex-direction | column | column | 1023=column/1024=row | row | - |
| 표지 넘침 | 0 | 0 | 0 | 0 | - |
| Q2 6지선다 카드 | - | - | - | 6개, 넘침 0 | 6개, 51px, 넘침 0 |
| Q4 캡션 | - | - | - | 정상 렌더, 쉼표 없음 | - |

Q2 는 5지에서 6지로 늘었지만 옵션 카드가 `useFitScale` 로 자동 축소되어 세로 넘침 없이 들어간다(1440 기준
51px, 실측). 그 외 Q1(5점 척도)·Q3(5지) 는 문항 길이만 늘고 선택지 개수는 그대로라 별도 이슈 없음.
...
## 좁은 창 카드 잘림 + 결과 막대 값 간격 수정 (2026-09-21)

사용자가 실제 브라우저(전체화면 아님, Whale)에서 찍은 스크린샷 두 장 첨부. Q2 6지선다 마지막 카드(F)가
창 하단에 잘려 있었고, 결과 막대의 `0%`와 `1표`가 너무 붙어 있었다.

### 카드 잘림 원인

`useFitScale` 이 `--fit` 으로 폰트·패딩은 줄이지만 `OptionList`/`WordCloud` 의 `gap` 과 `marginTop` 은
고정값이라 줄지 않았다. 브라우저 창이 좁고 낮으면(예 1728x760, 전체화면이 아닌 일반 창) 이 고정 여백만으로도
공간을 다 써서, 폰트를 아무리 줄여도 마지막 카드가 패널 밖으로 밀렸다. `calc(... * var(--fit,1))` 로 감싸
gap·marginTop 도 같이 줄게 했고, 축소 반복도 8회(0.51배 바닥)에서 14회(0.32배 바닥)로 늘렸다.

검증(격리 하네스, 6지선다 기준): 1920x1080부터 1024x500까지 전부 잘림 0. 이전에는 1728x760 같은
평범한 창 크기에서도 잘렸을 것으로 추정된다(실제 재현 스크린샷 근거).

### 값 간격

`chartValueGap` 상한을 18px→32px 로 올렸다. 4K(3840px)에서 이 값이 다른 vw 스케일 값들에 비해 너무 일찍
클램프돼 상대적으로 좁아 보였다. 퍼센트 숫자에 `min-width: 3.4ch` + 우측 정렬을 줘서 0%/20%/80%/100% 처럼
자릿수가 달라도 뒤따르는 `N표` 시작 위치가 행마다 흔들리지 않게 했다(실측: 5행 전부 `votesLeft` 동일값).

실측(운영, 20%/80%/0%/0%/0% 분포): 간격 23px(1920px 기준, 이전 대비 확대), 표 시작 위치 5행 모두 1313px 로 완전 일치.
이 값 열 코드는 BarChart 하나뿐이라 Q1(척도)·Q2·Q3 결과 화면에 전부 동시 적용됐다.

산출물: `bar-spacing-live.png`.


## 질문 자동 오픈 + 척도 슬라이더 눈금·기본값 (2026-09-21)

### 투표 열기 단계 제거

사용자 피드백: "시간도 없는데 거기에서 투표 열리는 걸 따로 하는 게 아니라 처음부터 질문 누르면 자동으로
열리게." 어드민이 질문을 클릭(`set_question`)하면 `voting_open` 이 바로 `true` 로 켜진다. 이전엔
질문 설정과 투표 열기가 별도 클릭 두 번이었다.

```
before: case 'set_question': return { ..., voting_open: false, ... };
after:  case 'set_question': return { ..., voting_open: true, ... };
```

"투표 닫기" 토글은 그대로 남아 있다. 마감하고 싶을 때만 누르면 된다(응답을 다 걷은 뒤 결과 공개 전에
멈추고 싶은 경우 등). `api/_lib/session-actions.js` 는 프로덕션 serverless 와 로컬 dev 미들웨어가
공유하는 단일 소스라 코드 한 곳만 고치면 양쪽에 다 반영된다.

### 척도 슬라이더 눈금

사용자가 실제 청중 화면 스크린샷 첨부. 트랙이 거의 안 보이는 연한 회색 한 줄뿐이라 "척도가 안 보이고
바도 안 보여서 사람들이 모를 거 같다"는 피드백. 0.5 단위 9개 정지점을 `I I I I I I I I I` 형태로
직접 표시했다.

- `/vote` `Slider`: 채워진 구간(파란 배경) 위의 눈금은 흰색, 빈 구간은 `ink2`(미선택 테두리 전용 색)로
  칠해 둘 다 보이게 했다. 트랙 높이(8px)보다 1.75배 높게 튀어나오게 해 거리 두고 봐도 잘 보인다.
- `/screen` `ScaleHint`(결과 공개 전 큰 화면): 같은 9개 눈금을 정적으로(채움 구분 없이) 추가해 청중이
  폰을 보기 전에도 "이건 눈금이 있는 척도구나"를 대형화면에서 먼저 읽을 수 있게 했다.

### 기본값 3

"처음에 3 기준으로 디폴트로 놔둬야 할 거 같아." 슬라이더 초기값을 `null`(선택 안 함)에서 `3`(중간)으로
바꿨다. 썸이 처음부터 중앙에 보이고, 그대로 제출하면 3점으로 들어간다(중립 응답도 유효한 응답이라는
설문 슬라이더의 일반적인 관례를 따랐다). 이전엔 값이 없으면 제출 버튼이 비활성이었는데, 이제 척도
질문은 항상 값이 있으니 처음부터 활성 상태다. 질문이 바뀌어도(다음 척도 질문 진입 시) 3으로 리셋된다.

### 검증 (운영 프로덕션)

| 항목 | 결과 |
|---|---|
| `set_question` 패치 후 `voting_open` | `true`(재현: DB 직접 patch로 set_question 결과 시뮬레이션) |
| `/vote` 진입 시 `aria-valuenow` | `3`(추가 조작 없이) |
| `/vote` 진입 시 제출 버튼 | 활성(비활성 아님) |
| `/vote` 눈금 개수 | 9 |
| `/screen` 힌트 눈금 개수 | 9 |

빌드 성공, 금지 항목 grep 0건. 테스트 후 votes 0행 / 세션 표지 복귀 확인.

산출물: `slider-ticks.png`, `screen-hint-ticks.png`.


## 척도 눈금 위치 수정 + 햅틱 (2026-09-21)

### 눈금 재배치

직전 커밋에서 넣은 눈금(트랙 안에 점으로 새김)이 의도와 달랐다. 사용자 피드백: "저렇게 하라는 게
아니라 바 위에다가 해달라고. 그리고 0.5 이런 숫자도 표시를 해줘야지. 저기 바 자체에 하는 건 좀 그렇고."
바 안에 새기지 않고, **바 위에 별도 줄로 1/1.5/2/2.5/3/3.5/4/4.5/5 숫자 9개**를 뒀다.
`/vote` `Slider`, `/screen` `ScaleHint` 둘 다 같은 방식으로 바꿨다.

검증(격리 하네스, /frame?path=/vote): 390px 모바일 실측 겹침 0, 가로 넘침 0, 9개 숫자 전부 표시.
1920px `/screen` 힌트도 동일 확인.

### 햅틱 피드백

사용자 요청: "사람들 폰에 햅틱이랑... 진동이랑 햅틱도 다." `navigator.vibrate` 로 세 곳에 넣었다.

- 슬라이더 눈금이 바뀔 때마다 짧게(10ms). 같은 값이 유지되는 동안은 울리지 않는다(드래그 중 연속
  진동 방지, 눈금 변경 시에만).
- 객관식 선택지 탭 시 짧게(8ms).
- 투표 제출 성공 시 짧게-쉼-짧게 패턴([12,40,12]ms)으로 다른 상황과 구분되는 확정 피드백.

**중요한 제약**: Vibration API 는 안드로이드 Chrome 계열에서만 동작한다. **아이폰 사파리는 이 API 자체가
없어 웹에서는 절대 작동하지 않는다**(우회 불가, 네이티브 앱이 아닌 이상). 이 사실을 사용자에게 먼저
확인받고 진행했다("햅틱 지원 범위: 그대로 진행"). `vibrate()` 헬퍼가 `navigator.vibrate` 존재 여부를
먼저 체크해 미지원 브라우저에서는 조용히 아무것도 안 하고 에러도 안 난다.

검증(격리 하네스): `navigator.vibrate` 를 모킹해 포인터다운으로 슬라이더 값이 바뀌자 `vibrate(10)` 호출
1회 확인.

빌드 성공, 금지 항목 grep 0건. 테스트 후 votes 0행 / 세션 표지 복귀.

산출물: `ruler-390.png`, `screen-hint-ruler.png`, `final-live-ruler.png`.


## 클로징 로고 색 + 모바일 여백 (2026-09-21)

### 클로징 로고가 안 보이던 문제

사용자 피드백: "마지막에 경주 그거... 텍스트가 화이트라 배경에 안 보여. 블랙으로 해줘."
원본 `wgj-2026.png` 를 픽셀 단위로 확인하니 **불투명 픽셀 43,273개가 전부 밝기 255(순백)** 였다.
배경이 밝은 ambient 색면이라 글자가 사실상 보이지 않는 상태였다.

알파 채널은 그대로 두고 RGB 만 `ink`(#0A0A0A)로 치환한 `wgj-2026-ink.png` 를 만들어 교체했다.
안티에일리어싱된 가장자리는 이미 알파에 반영돼 있어 RGB 만 바꿔도 자연스럽게 남는다. 원본은
보관용으로 남겨 뒀다(어두운 배경에 쓸 일이 생기면 그대로 쓸 수 있다).

참조 3곳 갱신: `Screen.jsx`, `SOURCE.md`, `DESIGN.md`.

### 모바일 질문 아래 여백

사용자 피드백: "390에서 지금 질문들이 기니까 질문 아래에 투표든 약간씩 여백을 줘가지고 좀 편하게."
큐시트 재확인으로 질문이 길어져 390px 에서 **질문이 4줄까지** 내려가는데, 답변 영역 상단 여백이
`spacing.panel`(20px)이라 질문과 선택지가 붙어 보였다. `spacing.2xl`(32px)로 넓혔다.

슬라이더(Q1), 주관식 입력(Q4), 선택지 목록(Q2/Q3), 종료 문구 네 군데 전부 같은 값으로 적용했다.

실측(격리 하네스, 390x844): 질문 4줄 → 답변 영역 간격 32px, 6지선다 기준 세로 넘침 0.
척도 화면도 동일하게 32px, 넘침 0.

산출물: `closing-ink-live.png`, `vote-390-spacing.png`, `vote-390-scale.png`.


## 투표가 집계에 반영되지 않던 버그 (2026-09-21)

### 증상

사용자가 폰으로 Q1 에 투표했는데 대형화면에 "1명 참여"만 뜨고 막대는 전부 0표였다.

### 원인

DB 에 남아 있던 그 표를 직접 열어 보니 이랬다.

```
question_id: Q1(=scale 타입)
option_id:   22222222-0001-...0005  ← 값이 들어있다
scale_value: null                    ← 비어 있다
```

Q1 을 choice → scale 로 바꾸기 전의 **예전 페이지가 폰에 그대로 떠 있었다.** 그 화면은 scale 타입을
모르니 남아 있던 옵션 2개를 선택지 버튼으로 그렸고, 투표를 `option_id` 로 보냈다. 서버는 그대로 받았다.

그다음이 문제였다. 두 집계 함수의 기준이 서로 달랐다.

| 함수 | 세는 대상 | 결과 |
|---|---|---|
| `get_results`(scale 분기) | `scale_value` 가 있는 행만 | 0표 |
| `get_live_count` | 질문의 **모든** 행 | 1명 |

그래서 "참여 수는 늘어나는데 막대는 0" 이라는 증상이 나왔다. 실측으로 재현 확인했다
(`get_results.total = 0`, `get_live_count = 1`).

### 수정 (0011_vote_shape_guard.sql, 운영 적용 완료)

1. **형태 검증 트리거**: `BEFORE INSERT` 로 질문 타입과 맞지 않는 투표를 거부한다(errcode `22023`).
   CHECK 제약으로는 다른 테이블(`questions.type`)을 참조할 수 없어 트리거로 만들었다.
   표가 조용히 사라지는 대신 청중 화면에 안내가 뜬다.
2. **참여 수 기준 통일**: `get_live_count` 를 타입별 집계와 같은 조건으로 바꿨다. 이제 참여 수와
   막대 합이 어떤 상황에서도 어긋나지 않는다.
3. **클라이언트 안내**: `submitVote` 가 `22023` 을 `stale` 로 구분하고, `/vote` 가
   "화면을 새로고침한 뒤 다시 한 번 눌러주세요" 를 띄운다.
4. **고아 표 삭제**: 집계에서 빠진 그 표 1건을 지웠다. 남겨두면 `unique(question_id, voter_key)`
   때문에 **새로고침하고 다시 투표해도 "이미 투표했어요" 로 막힌다.** 집계에도 안 잡히는데 재투표도
   막히는 최악의 상태라 반드시 지워야 한다.

### 검증 (운영 프로덕션)

| 항목 | 결과 |
|---|---|
| scale 질문에 `option_id` 로 투표 | 400, `vote_shape_mismatch` (거부) |
| scale 질문에 `scale_value` 로 투표 | 201 |
| 수정 후 `get_results.total` / `get_live_count` | 1 / 1 (일치) |
| 실제 UI 로 슬라이더 4.5 제출 → 대형화면 | 4.5 행이 100% 1표, "1명 참여, 평균 4.5점" |

### 행사 당일 주의

**질문 구성이나 타입을 바꾼 뒤에는 이미 열려 있던 청중 폰 화면이 낡은 상태로 남는다.** 이번처럼
타입이 바뀐 경우 그 화면은 제대로 투표할 수 없다. 이제는 조용히 실패하지 않고 새로고침 안내가 뜨지만,
QR 을 다시 스캔하거나 새로고침하면 확실하다. 행사 시작 전 최종 배포를 마친 뒤 QR 을 여는 순서를 지키면
이 상황 자체가 생기지 않는다.

산출물: `vote-fix-proof.png`.

## 클로징 화면 신설 + 경주포럼 분리 + 새로고침 바텀시트 (2026-09-22)

### 전체 리셋

운영 DB 를 처음 상태로 되돌렸다. votes 0행, status `cover`, `voting_open`/`results_visible` false,
`active_question_id` null, `results_view` bars.

### 마지막 화면을 둘로 나눴다

지금까지 `closing` 하나가 경주포럼 홍보였다. 이제 마지막 순서가 둘이다.

| status | 화면 | 내용 |
|---|---|---|
| `wgj` | 경주포럼 | 2026 세계경주포럼 로고 + 참가등록 QR (기존 closing 내용 그대로) |
| `closing` | 클로징 | 청중이 사진 찍는 배경. 새로 만든 화면 |

`0012_wgj_state.sql` 로 status CHECK 에 `wgj` 를 더했다(비파괴). **이름을 옮기지 않고 값을 더한 이유**:
이미 `closing` 을 쓰는 코드·문서가 있어 이름을 바꾸면 그 사이 배포된 클라이언트가 빈 화면을 만난다.
어드민 버튼도 "경주포럼 화면" / "클로징 화면" 두 개로 나란히 뒀다.

### 클로징 화면

**이 앱에서 유일하게 ambient 색면·글래스를 쓰지 않는 화면이다.** 사진에 담길 배경이라 그라데이션이
있으면 인물과 섞인다. `colors.paper`(#FEFEFE) 단색으로 깔았고, `GlassRoot` 바깥에서 조기 반환으로
그린다(같은 래퍼 안에 두면 뒤에 배경 이미지가 깔린다).

**브랜드 자산은 여기서만 원본 유채색을 쓴다.** 다른 화면은 `brandRamp` 재도색본이지만 클로징은 행사
아이덴티티를 그대로 남긴다. 사용자가 준 `자산 1logo.svg`(UIA 유채색), `자산 3graphic.svg`,
`HYU_logotype_blue_kor_eng.svg` 를 `brand-src/` 로 정리하고 `npm run brand` 산출물에 추가했다.

구성(위에서 아래로): 제3회 대한민국 사회적 가치 페스타 → 포럼 타이틀(`title.svg`, 커버와 같은 자산)
→ "대학의 미래, 미래의 대학" → (우측 그래픽) → 하단 UIA·한양대 로고 2개.
커버에 있던 `MODULE 05` / `인터렉티브 세션` 두 줄은 없앴다. 가운데 빈 공간이 사람이 서는 자리다.

**중복 자산 정리**: `자산 3graphic.svg` 는 기존 `brand-src/graphic.svg` 와 md5 가 완전히 같았다
(원래부터 유채색 원본이고 재도색은 메모리에서만 한다). 568KB 를 두 벌 두지 않고 한 파일을 공유한다.

### 새로고침 바텀시트

오래된 화면에서 투표해 서버가 거부할 때(0011) 인라인 문구 대신 바텀시트를 띄운다.
PATTERNS.md 규약이 "청중 화면 모달은 바텀시트"라 중앙 다이얼로그를 쓰지 않았다. 새로고침 버튼을 넣어
바로 복구할 수 있다. 딤 배경 `zIndex.overlay`, 시트 `zIndex.modal`, 등장은 transform + opacity 만 쓴다.

검증: 제출 요청을 가로채 `22023` 을 돌려주자 시트가 뜨고 제목 "화면이 오래됐어요", 새로고침 버튼 확인.

### 검증 (운영 프로덕션)

| 항목 | 결과 |
|---|---|
| 클로징 배경 | `rgb(254, 254, 254)`, glass-root 없음 |
| 클로징 자산 4종 로드 | title.svg / graphic-color.webp / logo-color.svg / hyu-logotype.svg 전부 ok |
| 클로징 구성 순서 | 행사명 → 포럼 타이틀 → 세션 타이틀 → 그래픽 → 로고 2개 |
| 경주포럼 화면 | 글래스 패널 유지, wgj-2026-ink.png + QR 정상 |
| 1920 / 390 넘침 | 가로·세로 모두 0 |

산출물: `closing-live.png`, `wgj-live.png`, `closing-390.png`, `stale-sheet.png`.


## 공동 1위 강조 + 루트를 투표 화면으로 (2026-09-22)

### 공동 1위도 blue

기존 규칙은 "단독 1등일 때만 blue, 동점이면 전부 inkSoft" 였다. 의도는 "1등이 없다는 사실을 색으로
드러낸다" 였는데, 실제로는 **공동 1위가 나오면 화면에서 1위를 아예 읽을 수 없었다.** 최다값 항목
전부를 blue 로 칠하도록 바꿨다. 0표(전부 0)일 때만 강조가 없다.

```
before: const highlight = top > 0 && leaders === 1 ? top : null;
after:  const highlight = top > 0 ? top : null;
```

막대·척도·워드클라우드가 같은 `highlight` 를 쓰므로 세 화면에 동시에 적용된다.
실측(3/3/1/0/0/0 분포): 1·2행 모두 blue, 3행 이하 inkSoft.

### 루트를 투표 화면으로 (현장 치명적 문제)

**증상**: 모바일에서 청중이 우하단 "메뉴"를 눌러 "투표 화면"으로 이동해야만 투표할 수 있었다.

**원인**: `/` 가 대형화면(`Screen`)이었다. 원래 설계 의도는 "청중은 QR 로만 들어오므로 도메인을 직접
치는 사람은 운영자뿐" 이었는데, 현장에서는 청중이 QR 대신 도메인을 직접 입력하거나 공유된 링크로
들어오는 경우가 많았다. 그러면 대형화면이 뜨고, 투표하려면 메뉴를 거쳐야 한다. 게다가 그 메뉴로
청중이 대형화면·관리자로 넘어갈 수도 있었다.

**수정**:
- `/` 를 `Vote` 로 바꿨다(리다이렉트 아님. 주소창 유지 + realtime 소켓 재연결 방지).
- `NavFab` 의 허용 경로에서 `/` 를 뺐다. 이제 청중이 보는 어떤 경로에도 메뉴가 없다.
- 메뉴의 "대형화면" 항목이 `/` 대신 `/screen` 을 가리킨다.
- 운영자는 `/screen` 으로 직접 들어가고, 거기서 메뉴로 관리자·전체화면을 쓴다.
...
| `/` (모바일) | 없음 | 있음 |
| `/vote` (모바일) | 없음 | 있음 |
| `/screen` (1920) | 있음 | 결과 화면 |

운영 배포 후 재확인: `/` 메뉴 없음 + 제출 버튼 있음, `/screen` 메뉴 있음 + 항목 3개(대형화면/관리자/투표 화면).

**행사 당일 주의**: 대형화면은 반드시 `https://uia-vote.vercel.app/screen` 으로 연다. 도메인만 치면
투표 화면이 뜬다. PROGRESS 맨 위 운영자 가이드도 같이 고쳤다.

산출물: `tie-blue.png`, `root-is-vote.png`.

## Q4 안내 문구 중앙 대형화 + 주관식 결과 자동 공개 (2026-09-22)

### 빈칸 문장을 화면 한가운데 크게

"내가 생각하는 미래의 대학은 ______이다." 가 질문 옆 작은 보조 문구(`screenMeta`)로 붙어 있어
20m 거리에서 읽히지 않았다(사용자 피드백). 질문 옆에서 떼어내 본문 자리에 중앙 정렬로 크게 띄운다.
`screenPrompt` 토큰 신설(clamp(30px, 4.6vw, 92px), weight 700). 1920 기준 88px.
단어가 하나라도 들어오면 이 자리를 워드클라우드가 대신한다.

### 주관식은 결과를 자동 공개

`set_question` 이 질문 타입을 조회해 `text` 면 `results_visible` 을 함께 켠다. 단어가 하나씩 쌓이는
과정 자체가 볼거리라 "결과 공개"를 따로 누를 이유가 없다(사용자 요청). 객관식·척도는 기존대로
공개 전까지 감춘다.

실측(운영): Q4 선택 → 응답 0개일 때 중앙 대형 문구 66px(1440 창 기준), 응답 3건 주입 후 워드클라우드로
자동 전환("연결" 2표 크게, "자율" 1표), 문구 사라짐, "3명 참여" 표시.

### 클로징 그래픽

"더 크게" 요청으로 전용 토큰을 만들어 키웠으나 하단 로고가 화면 밖으로 밀리는 문제가 반복돼
**표지와 동일하게 되돌렸다**(사용자 지시: "그냥 표지랑 똑같이 하라고"). 커버 토큰을 그대로 쓴다.
1920 실측 420x813 으로 표지와 픽셀 단위로 같다.

**이 과정에서 실수가 있었다.** 파이썬 문자열 치환으로 컴포넌트를 갈아끼우다 `WordCloud` 와
`OptionList` 두 함수를 통째로 날렸다. 화면이 빈 채로 렌더돼(루트 innerHTML 길이 0) 발견했고,
HEAD 에서 두 함수를 복원했다. 이후 9개 화면 상태(표지·QR·선택지·공동1위·척도·Q4 응답0·Q4 응답20·
경주포럼·클로징)를 전부 다시 확인해 렌더·넘침 0 을 검증했다.


## 결과 아카이브 + 행사 종료 처리 (2026-09-22)

### 행사 실데이터

실제 행사 응답이 DB에 남아 있다. **삭제하지 않는다.**
...
`votes` 행은 자동으로 지워지지 않는다. 삭제 경로는 어드민 "이 질문 초기화"/"전체 초기화" 두 개뿐이고,
"세션 종료"는 아무것도 지우지 않는다. 앞으로 이 프로젝트에서 votes 전체 삭제를 실행하지 않는다.

### 세션 중지

`status = 'ended'`, `voting_open=false`, `results_visible=false` 로 바꿨다. 투표 200건은 그대로다.
청중 화면(`/`, `/vote`)은 "오늘 세션이 종료되었어요" 를 띄운다.

**알려진 동작**: `/screen` 에는 `ended` 전용 분기가 없어 마지막 질문 화면이 남는다. 대형화면을 더 이상
띄우지 않으므로 이번에는 문제가 되지 않았다. 다음에 쓸 일이 있으면 표지(`cover`)로 돌리면 깔끔하다.

### 결과 아카이브 `/archive`

행사가 끝난 뒤 질문별 최종 결과를 보고 이미지로 남기는 화면이다.

- 세션 상태와 무관하게 DB 집계(`get_results`, `get_live_count`)를 직접 읽는다. 운영 화면에 영향이 없다.
- 카드마다 **실제 대형화면(`ScreenView`)을 1920x1080 그대로** 그리고 카드 폭에 맞춰 축소해 보여준다.
  아카이브용 결과 레이아웃을 새로 만들지 않았다. 행사장에서 보던 화면이 그대로 기록으로 남아야 한다.
- "이미지로 저장" 은 `html-to-image` 로 원래 크기를 pixelRatio 2 로 캡처해 **3840x2160 PNG** 를 만든다.
  파일명은 `질문1-결과.png` 형식. 캡처 직전 `transform: none` 으로 축소를 끈다.
- 상단에 질문 수와 응답 합계, 카드마다 참여 수를 함께 적는다.
- 글래스는 CSS 폴백 고정. 카드가 여러 장이라 WebGL 컨텍스트 한도를 넘기지 않기 위해서다(/preview 와 같은 이유).
- `NavFab` 메뉴에 "결과 아카이브" 추가.

검증(운영): 4개 카드 렌더, 합계 200건 표시, Q1·Q4 이미지 실제 생성 확인(각각 889KB / 1.26MB, 3840x2160).
세션을 `ended` 로 바꾼 뒤에도 아카이브는 그대로 동작한다.

### 같이 고친 것

- **제출 버튼 하단 고정**(`sticky bottom-lg z-sticky`). 질문이 길고 선택지가 6개면 375x667 에서 버튼이
  화면 밖으로 151px 밀려 스크롤해야 보였다. PATTERNS.md 에 이미 "하단 고정" 으로 적혀 있었는데 구현이 빠져 있었다.
  sticky 를 버튼에 직접 걸면 `.glass` 의 `position: relative` 가 이겨서 래퍼 div 에 건다.
- **클로징 높이 확정**(`h-dvh`). `min-h-dvh` 로는 안쪽 flex 높이가 확정되지 않아 세로가 짧은 창에서
  타이틀이 줄어들 여지를 못 얻고 하단 로고가 밀렸다(실측 1280x600 에서 106px 초과). 1920 모습은 그대로다.

### 점검에서 확인한 것

- 금지 항목 0건(TypeScript, localStorage, HEX 하드코딩, hover:scale, transition-all, 네이티브 select/date, 이모지, em-dash, 임의 z-index)
- 토큰 참조 누락 0건, 문서가 참조하는 토큰 누락 0건, 루트 `tokens.js` 는 재수출 포인터 유지
- 마이그레이션 0001~0012 전부 `_ALL.sql` 에 포함, 운영 DB 스키마와 일치
- dist 격리: `__dev__`, DevControlPanel, Preview, 패스코드 문자열 전부 0건
- 대형화면 7개 상태 x 5개 해상도(3840/1920/1440/1280/390) 렌더·넘침 점검 통과
